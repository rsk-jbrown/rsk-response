...
class CtfFeedPro extends CtfFeed
{
	...
	public static function init( $atts, $last_id_data = '', $num_needed_input = 0, $ids_to_remove = array(), $persistent_index = 1 )
	{
		...
		$feed->setStandardTextOptions( 'gdpr', 'auto' );
		...
		if ( CTF_GDPR_Integrations::doing_gdpr( $feed->feed_options ) ) {
	...
	private function persistentCacheTweets()
	{
		...
		if ( ! empty( $this->last_id_data ) || ( ! $cache_time_limit_reached && $existing_cache ) ) {
			...
			if ( empty( $tweet_set ) ) {
				...
				update_option( $cache_name, $cache_set, false );
				return $existing_cache;
			} else {
			...
			if ( $this->tweet_set ) {
				...
				update_option( $cache_name, $tweet_set, false );
	...
	public function maybeSetTweetsFromTwitter()
	{
		...
		if ( empty ( $this->feed_options['feed_types_and_terms'] ) || ! isset( $this->feed_options['feed_types_and_terms'] ) ) {
			...
			if ( ! isset( $working_tweet_set['errors'][0] ) && isset( $working_tweet_set[0] )
	...
	public function getTweetSetHtml( $is_pagination = 0 )
	{
		...
		$ctf_feed_classes = CTF_Parse_Pro::get_feed_classes( $feed_options, $this->check_for_duplicates );
		$cols_att = CTF_Display_Elements_Pro::get_cols_atts( $feed_options );
		$ctf_main_atts = $ctf_data_disablelinks . $ctf_data_linktextcolor . $ctf_data_autoscrolldistance . $ctf_enable_intents . $cols_att;
		$ctf_data_atts = $ctf_data_other . $ctf_data_maxmedia . $ctf_data_imagecols;
		$ctf_data_atts .= ' data-feedid="' . esc_attr( $feed_id ) . '"';
		$flags_att = '';
		$flags = array();
		if ( CTF_GDPR_Integrations::doing_gdpr( $feed_options ) ) {
			$flags[] = 'gdpr';
			if ( ! CTF_GDPR_Integrations::blocking_cdn( $feed_options ) ) {
		...
		$ctf_data_atts .= $flags_att;

		$ctf_data_atts .= ' data-postid="' . esc_attr( get_the_ID() ) . '"';

		if ( ! $is_pagination ) {
            ob_start();
                include ctf_get_feed_template_part( 'feed', $options );
                $tweet_html .= ob_get_contents();
            ob_get_clean();
        } 
        /*
        if ( $is_pagination > -1 && ( isset ( $tweet_set[1]['id_str'] ) ) ){
            while( $i < $len ) {
	            $media = CTF_Parse_Pro::get_media( $tweet_set[ $i ], false );
	            $quoted_media = CTF_Parse_Pro::get_quoted_media( $tweet_set[ $i ], false );
	            if ( $media ) {
		            $ids_in_set_w_media[] = CTF_Parse_Pro::get_post_id( $tweet_set[ $i ] );
	            } elseif ( $quoted_media ) {
		            $ids_in_set_w_media[] = CTF_Parse_Pro::get_post_id( $tweet_set[ $i ] );
	            }

	            ob_start();
                    include ctf_get_feed_template_part( 'item', $options );
                    $tweet_html .= ob_get_contents();
                ob_get_clean(); 
                $i++;
            }
        ...
        if ( ! $is_pagination ) {
                ob_start();
                    include ctf_get_feed_template_part( 'footer', $options );
                    $tweet_html .= ob_get_contents();
                ob_get_clean();
        }
*/
	...
	public function tweet_loop( $tweet_set, $settings, $is_pagination ) {
		...
		$i = $is_pagination; // starts at index "1" to offset duplicate tweet
		$ids_in_set_w_media = array();
        $t = ( count($tweet_set) > 1 ? $t = 1 : 0 ); // Set index based on tweets returned
